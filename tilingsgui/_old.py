...
# Just adding this here so I can run the old one easily with this venv that uses dev branch of Tilings
...
window = pyglet.window.Window(
    height=INITIAL_HEIGHT, width=INITIAL_WIDTH, resizable=True
)
...
def draw_circle(pos, r, color=[0, 0, 0]):
    ...
    pyglet.graphics.draw(
        N + 2, pyglet.gl.GL_TRIANGLE_FAN, ("v2f", verts), ("c3B", cols)
    )
...
def draw_line_segment(p1, p2, color=[0, 0, 0]):
    ...
    verts = [x1, y1, x2, y2]
    cols = color * N
    pyglet.graphics.draw(N, pyglet.gl.GL_LINE_STRIP, ("v2f", verts), ("c3B", cols))
...
def draw_filled_rectangle(loc, sz, color=[0, 0, 0]):
    ...
    cols = color * N
    pyglet.graphics.draw(N, pyglet.gl.GL_TRIANGLE_STRIP, ("v2f", verts), ("c3B", cols))
...
class TilingDrawing(object):
    def __init__(self, tiling, x, y, w, h):
        ...
        self.obstruction_locs = [
            gridded_perm_initial_locations(gp, (tw, th), (self.w // tw, self.h // th))
            for gp in self.tiling.obstructions
        ]
        self.requirement_locs = [
            [
                gridded_perm_initial_locations(
                    gp, (tw, th), (self.w // tw, self.h // th)
                )
                for gp in reqlist
            ]
    ...
    def draw(self):
        ...
        for i, reqlist in enumerate(self.requirement_locs):
            ...
            col = (
                HIGHLIGHTED_COLOR
                if hover_index != None and i == hover_index[0]
                else REQ_COLOR
            )
...
def on_mouse_drag(x, y, dx, dy, button, modifiers):
    ...
    if selected_point == None:
        ...
        if move_type == "point":
            tiling_drawing.obstruction_locs[i][j] = (
                clamp(x, mnx, mxx),
                clamp(y, mny, mxy),
            )
        else:
        ...
        mnx, mxx, mny, mxy = point_move_bounds
        tiling_drawing.requirement_locs[i][j][k] = (
            clamp(x, mnx, mxx),
            clamp(y, mny, mxy),
        )
...
def on_key_press(symbol, modifiers):
    ...
    if symbol == pyglet.window.key.S and modifiers & pyglet.window.key.MOD_CTRL:
        try:
            ...
            print(
                "Unicode error, make sure your terminal/font support unicode then try running the command:"
            )
